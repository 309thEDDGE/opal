...
test_artifacts_path = '/home/jovyan/opal/opal-packages/tests/flow/test_artifacts.py'
...
def test_midnight_flow_torch_create_singleuser_read():
    ...
    run = subprocess.run([singleuser_python_path, test_artifacts_path], capture_output=True)
    if run.returncode != 0:
...
def test_midnight_flow_singleuser_create_torch_read():
    ...
    run = subprocess.run([torch_python_path, test_artifacts_path], capture_output=True)
    if run.returncode != 0:
        print( 'exit status:', run.returncode )
        print( 'stdout:', run.stdout.decode() )
        print( 'stderr:', run.stderr.decode() )
