...
class Chapter10Catalog(opal.flow.OpalFlowSpec):
    ...
    cache = metaflow.Parameter(
        "cache",
        help="Use latest successful run as cache",
        default=True
    )
        ...
        for source_name, source_spec in self.sources.items():
            ...
            for ch10_p in ch10_paths:
                ...
                if ch10_url in cached_dict:
                    ...
                    if ch10_size == cached_record['size'] and source_name == cached_record['source']:
                        print(f"{log_prefix} Found {ch10_name} in cache ({ch10_size} b)")
