...
class TipTranslateFlow(opal.flow.OpalFlowSpec):
    parse_pointer = metaflow.Parameter(
        "parsed",
        help="Metaflow run ID of the parsed data to translate",
        required=True
    )
    ...
    temp_dir = metaflow.Parameter(
        "temp-dir",
        default=os.path.expanduser("~/translated_data")
    )
    
    dts = metaflow.Parameter(
        "dts",
        help="ICD to translate the parsed data with"
    )
